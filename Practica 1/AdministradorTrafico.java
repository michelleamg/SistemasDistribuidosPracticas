...
public class AdministradorTrafico {
    public static void main(String[] args) {
        if (args.length != 5) {
            System.out.println("Uso: java AdministradorTrafico <puerto_proxy> <ip_servidor1> <puerto1> <ip_servidor2> <puerto2>");
            return;
        }
        ...
        try (ServerSocket serverSocket = new ServerSocket(puertoProxy)) {
            System.out.println("Proxy inverso escuchando en puerto " + puertoProxy);

            while (true) {
                Socket navegador = serverSocket.accept();
    ...
    private static void manejarConexion(Socket navegador, String ipServidor1, int puertoServidor1,
